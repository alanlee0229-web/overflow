                ...
                for box in boxes:
                    ...
                    if class_id == self.pot_class_id:
                        ...
                        expanded_x1, expanded_y1, expanded_x2, expanded_y2 = self.expand_bbox(
                            x1, y1, x2, y2, img_width, img_height, expand_ratio=1.08
                        )
...
def process_video_for_pots(video_path, output_dir, pot_detector, save_interval=1, show_preview=True):
    ...
    while True:
        ...
        if frame_count % save_interval == 0:
            ...
            for i, cropped_img in enumerate(cropped_images):
                ...
                filename = f"pot_{timestamp}_{frame_count:06d}_{i}.png"
            ...
            print(f"已处理 {frame_count}/{total_frames} 帧, 检测到 {len(cropped_images)} 个锅具, "
                  f"处理速度: {fps_processed:.2f} FPS", end='\r')
    ...
    print(f"\n处理完成! 共保存了 {saved_count} 张锅具图像, 总耗时: {total_time:.2f} 秒")


def process_videos_in_folder(input_folder, output_root_dir, model_path, save_interval=1, show_preview=True):
    """
    处理文件夹中的所有视频文件
...
def main():
    ...
    # 输入视频文件夹路径
    input_folder = r"F:\work_area\___overflow\data_videos\1012\0"  # 替换为您的视频文件夹路径

    # 输出根目录
    output_root_dir = r"F:\work_area\___overflow\data_img\data_1015"      # 所有视频的输出将存储在此目录下的子文件夹中
