...
# 导入多patch检测相关类
from multi_patch_detection import MultiPatchDetector, AttentionPatchDetector
...
CLASSIFICATION_MODEL_PATHS = [
    r"F:\work_area\___overflow\code_\mod_2_old\little_model\0117\model_V12_ori.pth",
    r"F:\work_area\___overflow\code_\mod_2_old\little_model\0117\model_V12.1a.pth",
    r"F:\work_area\___overflow\code_\mod_2_old\little_model\0117\model_V12.2b.pth"
]
...
# 结果报告输出路径
RESULT_OUTPUT_PATH = r"F:\work_area\___overflow\code_\mod_2_old\little_model\0117\charts\validation_results.txt"
# 可视化图表输出目录
CHART_OUTPUT_DIR = r"F:\work_area\___overflow\code_\mod_2_old\little_model\0117\charts"
...
# 多Patch检测配置
USE_MULTI_PATCH = True  # 是否使用多Patch检测
PATCH_STRATEGY = 'edge_focus'  # patch提取策略: edge_focus, grid, adaptive
VOTING_STRATEGY = 'any'  # 投票策略: any, majority, weighted
PATCH_SIZE = 112  # Patch大小
USE_ATTENTION_PATCH = False  # 是否使用基于注意力的自适应Patch检测
...
class ModelValidator:
    ...
    def load_models(self):
        ...
        for i, model_path in enumerate(self.classification_model_paths):
            try:
                ...
                # 3. 创建多patch检测器（如果启用）
                multi_patch_detector = None
                if USE_MULTI_PATCH:
                    multi_patch_detector = MultiPatchDetector(
                        model=classification_model,
                        device=self.device,
                        patch_strategy=PATCH_STRATEGY
                    )
                
                # 4. 创建注意力patch检测器（如果启用）
                attention_patch_detector = None
                if USE_ATTENTION_PATCH:
                    attention_patch_detector = AttentionPatchDetector(
                        model=classification_model,
                        device=self.device
                    )
                ...
                self.classification_models.append({
                    ...
                    'multi_patch_detector': multi_patch_detector,
                    'attention_patch_detector': attention_patch_detector,
                    'path': model_path,
                    'name': f"模型{i+1}",
                    'load_time': load_time
                })
    ...
    def predict_single_image(self, image_path):
        ...
        try:
            ...
            for model_info in self.classification_models:
                ...
                multi_patch_detector = model_info['multi_patch_detector']
                ...
                with torch.no_grad():
                    ...
                    if USE_MULTI_PATCH and multi_patch_detector:
                        # 使用多patch检测
                        # 确保输入是4D tensor: [batch, channels, height, width]
                        input_tensor = efficientnet_input.unsqueeze(0).to(self.device)
                        result = multi_patch_detector.predict(input_tensor, voting_strategy=VOTING_STRATEGY)
                        
                        # 计算整体溢出概率（基于patch结果）
                        patch_probs = [r['overflow_prob'] for r in result['patch_results'].values()]
                        overflow_prob = sum(patch_probs) / len(patch_probs)
                        
                        # 使用投票结果作为最终决策
                        is_overflow = result['is_overflow']
                        predicted_class_index = 1 if is_overflow else 0
                        confidence = overflow_prob if is_overflow else 1 - overflow_prob
                ...
                # 添加patch结果（如果有）
                if patch_results:
                    model_result['patch_results'] = patch_results
    ...
    def process_class_folder(self, class_folder_path, true_class, results):
        ...
        for i, image_file in enumerate(image_files):
            ...
            for model_result in model_results:
                ...
                # 显示多patch检测结果（如果有）
                if 'patch_results' in model_result and model_result['patch_results']:
                    print(f"      Patch检测结果:")
                    for patch_pos, patch_info in model_result['patch_results'].items():
                        print(f"        {patch_pos}: {'溢出' if patch_info['is_overflow'] else '未溢出'} (概率: {patch_info['overflow_prob']:.4f})")
    ...
    def print_summary(self, results):
        ...
        print(f"使用多Patch检测: {'是' if USE_MULTI_PATCH else '否'}")
        if USE_MULTI_PATCH:
            print(f"  Patch策略: {PATCH_STRATEGY}")
            print(f"  投票策略: {VOTING_STRATEGY}")
            print(f"  Patch大小: {PATCH_SIZE}")
        print(f"使用注意力Patch: {'是' if USE_ATTENTION_PATCH else '否'}")
...
def save_results_to_file(validator, all_results, output_path):
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  使用多Patch检测: {'是' if USE_MULTI_PATCH else '否'}\n")
            if USE_MULTI_PATCH:
                f.write(f"    Patch策略: {PATCH_STRATEGY}\n")
                f.write(f"    投票策略: {VOTING_STRATEGY}\n")
                f.write(f"    Patch大小: {PATCH_SIZE}\n")
            f.write(f"  使用注意力Patch: {'是' if USE_ATTENTION_PATCH else '否'}\n")
