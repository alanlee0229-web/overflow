...
# 导入优化相关类
from precision_recall_optimizer import AdvancedTTA, PostProcessor
        ...
        try:
            ...
            for model_info in self.classification_models:
                ...
                with torch.no_grad():
                    ...
                    if USE_TTA:
                        # 使用优化后的TTA推理
                        is_overflow, overflow_prob, _ = tta_inference.predict_with_tta(
                            image_tensor=efficientnet_input,
                            threshold=OVERFLOW_THRESHOLD,
                            num_augmentations=5,  # 生产环境建议3-5
                        ...
                        is_overflow = overflow_prob > OVERFLOW_THRESHOLD
                        predicted_class_index = 1 if is_overflow else 0
                        confidence = overflow_prob if predicted_class_index == 1 else 1 - overflow_prob
