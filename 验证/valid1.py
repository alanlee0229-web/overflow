...
class VideoPotDetector:
    ...
    def classify_pot(self, pot_image):
        """对整个锅具图像进行分类"""
        if not self.model_loaded or pot_image.size == 0:
            return None

        try:
            # 预处理图像
            # input_tensor = self.preprocess_image(pot_image)
            # 预处理图像
            blob = cv2.dnn.blobFromImage(pot_image, scalefactor=1 / 255, size=(224, 224), swapRB=True)
    ...
    def process_frame(self, frame):
        ...
        for i, result in enumerate(results):
            ...
            for j, box in enumerate(boxes):
                ...
                # 对整个锅具图像进行分类
                pot_result = self.classify_pot(pot_resized)

                # 在原图上绘制检测框
                if pot_result['class_name'] == 'boiling over':
                ...
                cv2.rectangle(frame, (x1_new, y1_new), (x2_new, y2_new), box_color, 2)

                # 在原图上显示分类结果和置信度
                result_text = f"{overall_class}: {pot_result['confidence']:.2f}"
                cv2.putText(frame, result_text, (x1_new, y1_new - 10),
...
# def main():
#     # 模型路径配置
#     yolo_model_path = r"E:\all_flow\scene\pot_424.pt"  # 替换为您的YOLO模型路径
#     efficientnet_model_path = r"E:\all_flow\model\efficientnet_b0_V10_10.11.pth"  # 替换为您的EfficientNet模型路径
#     address = [r"F:\all_flow\源哥\SpillOver_20250928_155913_ori.avi",
#                r"F:\all_flow\源哥\SpillOver_20250928_160121_ori.avi",
#                r"F:\all_flow\源哥\SpillOver_20250928_160208_ori.avi",
#                r"F:\all_flow\源哥\SpillOver_20250928_160403_ori.avi",
#                r"F:\all_flow\航哥\original_20250811145125.mp4",
#                r"F:\all_flow\航哥\original_20250813144822.mp4",
#                r"F:\all_flow\航哥\original_20250725104633.mp4",
#                r"F:\all_flow\航哥\original_20250723155750.mp4"]
#     # 视频文件路径
#     for path in address:
#         # video_path = r"F:\all_flow\航哥\original_20250725104633.mp4" # 替换为您的视频文件路径
#         video_path = path  # 替换为您的视频文件路径

#         # 输出目录
#         output_dir = "video_results1"
...
def main():
    ...
    efficientnet_model_path = r"F:\work_area\___overflow\code_\mod_2_old\1017new\efficientnet_b0_best_lkh1.pth"  
    ...
    for path in address:
        ...
        # 输出目录
        output_dir = r"F:\work_area\___overflow\video_output\1"
